...
{
    using Common;
    using Register = Common.CPU.Register;
    ...
    public class Day23 : IExercise
    {
        ...
        private Register GetRegister(CPU cpu, string register)
        {
            return (Register)cpu.GetType().GetProperty($"Register{register.ToUpper()}").GetValue(cpu);
        }
    }
}
